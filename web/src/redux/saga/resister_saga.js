...
function * fetchReigster(action) {
  try{
    const data = yield call(postData,`${myserver}/API/broadcast/accountCreate`,action.payload)
    yield put({
...
function * checkUserName(action){
  try{
    ...
    const data= yield call(postData,`${myserver}/API/api/getAccounts`,action.payload)
    yield put({
      type:POST_CHECKUSERNAME_SUCCESS,
      payload:data
    })
